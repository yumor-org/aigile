...
description: |
  ...
  対象 Issue の内容と既存 Requirement Document を読み取り、`.aigile/docs/requirements/<slug>.md` を新規作成または更新する PR を Draft で発行する。
...
対象 Issue を分析し、`.aigile/docs/requirements/<slug>.md` を **新規作成または更新する PR を Draft で発行** してください。
...
このリポジトリは AI ネイティブなアジャイル開発フレームワーク **aigile** を構築するプロジェクトです。あなたが担うのは aigile の 10 ステップフロー（`docs/workflow.md`）のうち、**ステップ 4 (Requirement Document の作成・更新 PR 発行)** に相当する役割です。Ready 判定（ステップ 3）は前段の `aigile-requirement-analyzer` ワークフローが既に完了させており、本 Issue は情報十分かつ既存 Document と非コンフリクトと判定済みです。

押さえるべき原則:

- **Requirement レイヤー = 「誰が、何を、なぜ望むか」** を記述する層（`docs/layers.md`）。実装方法ではなく **振る舞い** として要求を書く。
- **Requirement の承認は人間限定（不変条件）** （`docs/concepts.md`）。本 PR はあくまで Draft として人間レビュアーに提示するもので、AI による承認は行わない。
...
3. `bash` ツールで以下を確認する。
   - `cat .aigile/docs/templates/requirement.md` で Document テンプレートと PR テンプレートの最新仕様を読み込む。
   - `find .aigile/docs/requirements -type f -name '*.md' 2>/dev/null` で既存 Document の有無を確認する。
   - `date +%Y-%m-%d` で本日の日付を取得する（Document の `Last Updated` に使用）。
...
`.aigile/docs/templates/requirement.md` の「配置とファイル名」に従う:
...
4. 新規作成の場合のパス: `.aigile/docs/requirements/<slug>.md`

## 手順 3: Document 本文の生成

`.aigile/docs/templates/requirement.md` の「テンプレート」セクションのフォーマットに **厳密に** 従って Document を生成または更新する。主要セクション:

- タイトル行: `# Requirement: <Issue タイトルから [REQ] を除いたもの>`
- メタ表: `Issue` 番号、`Status: Draft`、`Last Updated: <YYYY-MM-DD>`
...
- **Status は `Draft` を維持**（人間レビュアーの承認まで変更しない）。
...
- `Last Updated` を本日の日付に置き換える。
- Status は既存値（通常 `Draft`）を維持。人間がマージ済みの Document を更新する場合でも、本 PR がマージされるまでは新規 Status を提案しない。
- 既存内容を尊重しつつ、Issue で新たに合意された事項を反映する。差分の根拠は PR 本文の「変更内容」で説明する。

## 手順 4: ファイル書き込みとブランチ準備

1. `edit` ツールで `.aigile/docs/requirements/<slug>.md` を作成または更新する。ディレクトリが存在しない場合は `mkdir -p .aigile/docs/requirements` で作成する。
...
`create-pull-request` 安全出力で、以下のメタデータの PR を Draft で 1 件だけ発行する。
...
PR 本文は `.aigile/docs/templates/requirement.md` の「PR 化のメタデータ」の本文テンプレートに従う。`<issue 番号>` と `<slug>` を実値に置換する:
...
- **書き換えてよいファイルは `.aigile/docs/requirements/<slug>.md` のみ**。`docs/` 配下や他の `.aigile/` 設定ファイルには触れない。
- **PR タイトルの `[Requirement] ` プレフィックスは安全出力側で自動付与される**。あなたが渡すタイトル引数にはプレフィックスを含めないこと（二重付与防止）。
- **Status は `Draft`**。Requirement レイヤーの承認は人間レビュアーがマージで行うため、本ワークフローでは Status を変更しない。
