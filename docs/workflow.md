...
|---|---|---|---|---|---|
| 1 | 要求の起票 | 人間が `Requirement Issue (追加の要求)` テンプレートで Issue を作成 | 人間 | `aigile:issue:requirement` ラベル付き Issue | (Issue テンプレート) |
| 2 | 要求の分析 | 対象 Issue のコメントで `aigile` をメンション | AI | Issue コメント（質問 or Ready 判定通知）／ `aigile:issue:requirement:ready` ラベル付与 | [aigile-requirement-analyzer](../.github/workflows/aigile-requirement-analyzer.md) |
| 3 | Document 化 | `aigile:issue:requirement:ready` ラベル付与 | AI | `[Requirement]` プレフィックス付き PR（`.aigile/docs/L1_requirements/<slug>.md`） | [aigile-requirement-doc-writer](../.github/workflows/aigile-requirement-doc-writer.md) |
| 4 | Requirement 承認 | 人間が PR をレビューしマージ | 人間 | Document on main + 起点 Issue Accepted-Closed | (GitHub 標準 PR フロー) |
| 5 | Spec 検証・更新 | Requirement Doc PR の merged | AI | Green: Spec PR ／ Amber: 起点 Requirement Issue にコメント ／ Gray: ログのみ | [aigile-specification-doc-writer](../.github/workflows/aigile-specification-doc-writer.md) |
| 6 | Specification 承認 | 人間 or AI が PR をレビューしマージ | `.aigile/stakeholders.yml` の設定に従う | Document on main | (PR フロー) |
| 7 | Arch 検証・更新 | Specification Doc PR の merged | AI | Green: Arch PR ／ Amber: 起点 Requirement Issue にコメント ／ Gray: ログのみ | [aigile-architecture-doc-writer](../.github/workflows/aigile-architecture-doc-writer.md) |
| 8 | Architecture 承認 | 人間 or AI が PR をレビューしマージ | `.aigile/stakeholders.yml` の設定に従う | Document on main | (PR フロー) |
| 9 | 実装計画 | （未実装、将来予定） | AI | 実装 Issue | (未定義) |
| 10 | 実装自律 PR | （未実装、将来予定） | AI | 実装 PR | (未定義) |

凡例:
- 列「アクター」: そのイベントを **進める主体**。人間アクションは GitHub UI 操作、AI アクションは `gh aw` ワークフローで自動実行される。
...
```mermaid
flowchart TD
    Start([1. 人: Requirement Issue 作成]) -->|aigile メンション| Analyze[2. AI: requirement-analyzer<br/>Issue 分析・質問]
    Analyze -->|未解決の質問あり| Analyze
    Analyze -->|Ready ラベル付与| ReqPR[3. AI: requirement-doc-writer<br/>Req Document PR]
    ReqPR --> ReqReview{4. 人: レビュー}
    ReqReview -->|却下| RejectClose([Requirement Issue:<br/>Rejected-Closed])
    ReqReview -->|マージ| ReqMerged([Document on main +<br/>Issue Accepted-Closed])

    ReqMerged -->|PR merged 発火| SpecAnalyze[5. AI: specification-doc-writer<br/>影響範囲 3 帯域分類]
    SpecAnalyze -->|Green / 新規作成| SpecPR[Spec Document PR]
    ...
    SpecMerged -->|PR merged 発火| ArchAnalyze[7. AI: architecture-doc-writer<br/>影響範囲 3 帯域分類]
    ArchAnalyze -->|Green / 新規作成| ArchPR[Arch Document PR]
    ...
    ArchMerged -.->|将来予定| Plan[9. AI: 実装計画<br/>実装 Issue 起票]
...
すべての PR / コメントは **起点 Requirement Issue 番号を本文中に明記** します。Spec PR は起点 Req の `source_issue` を直読み、Arch PR は Spec の `depends_on` を辿って Req に到達し、Req の `source_issue` を集約して列挙します。
