...
aigile では、プロジェクトの "満たすべき要求" は **Document** に集約されます。Document は **ベースブランチ**（デフォルト: `main`、[project-config.md](project-config.md) で設定可能）にマージされた状態が唯一の真実であり、他のすべての成果物（Issue, PR, コード）は Document を起点に解釈されます。
...
| ライフサイクル | 短命（Closed で役目を終える） | 永続（ベースブランチに残る） |
| 状態の終端 | Accepted-Closed / Rejected-Closed | マージ (=合意) / 改訂 |
| 認可主体 | 起票者は誰でも可 | レイヤーごとの承認者（[stakeholders.md](stakeholders.md) 参照） |

Issue は Git における Pull Request、Document はベースブランチの状態に類比できます。Issue は提案であり、Document は契約です。
...
flowchart LR
    ...
    subgraph per["永続 (Document on ベースブランチ)"]
        D[Document: 合意済み契約]
    end
    I -->|AI 作成| P
    P -->|レビュー・マージ| D
...
両者の境界判定はシンプルで、**対象 Document がベースブランチにマージ済みか否か** が判定基準になります。未マージなら通常の Requirement Issue / PR 内議論で処理し、マージ済みならエスカレーション Issue に進みます。
