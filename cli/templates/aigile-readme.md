...
| [docs/requirements/](docs/requirements/) | Requirement Document の格納先。マージされた Document が「合意済みの要求」を表す。 |
| [docs/templates/requirement.md](docs/templates/requirement.md) | Requirement Document の標準テンプレート。 |
...
aigile では Issue（提案・報告）と Document（合意済みの契約）を分け、ベースブランチにマージされた Document を Source of Truth として扱います。最初のサイクルは次の流れで進みます。

1. **人間**: `New issue` → `Requirement Issue (追加の要求)` で要求を起票する。
2. **AI**: Requirement Analyzer が Issue にコメントで質問を返す。Issue コメントで `@aigile` をメンションすると応答が起動する。
3. **人間**: AI の質問に Issue 上で回答し、要求の輪郭を確定させる。
4. **人間**: リリース計画のタイミングで、対象 Issue に `aigile:issue:requirement:ready` ラベルを付ける。
5. **AI**: Requirement Document（[docs/requirements/](docs/requirements/)）を起こす PR を発行する。
6. **人間**: PR をレビューしてマージすると、対応する Issue が Accepted-Closed として閉じられる。

10 ステップ全体・Specification / Architecture 以降の流れ・エスカレーション機構は [workflow.md](https://github.com/yumor-org/aigile/blob/main/docs/workflow.md) と [escalation.md](https://github.com/yumor-org/aigile/blob/main/docs/escalation.md) を参照してください。
...
Issue が立つと Requirement Analyzer が自動でコメントに質問を残します。以後、Issue コメントで `@aigile` をメンションすると再度応答します。要求が確定したらリリース計画のタイミングで `aigile:issue:requirement:ready` を付与します。
...
- [workflow.md](https://github.com/yumor-org/aigile/blob/main/docs/workflow.md) — 10 ステップの開発フロー
- [layers.md](https://github.com/yumor-org/aigile/blob/main/docs/layers.md) — 4 層 Document モデル
